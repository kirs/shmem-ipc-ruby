...
unsafe extern "C" fn float_sender_new(_klass: VALUE, capacity_val: VALUE) -> VALUE {
    ...
    match Sender::new(capacity) {
        Ok(sender) => {
            ...
            rb_ivar_set(obj, rb_intern(b"@ptr\0".as_ptr() as *const i8), rb_ull2inum(wrapper_ptr as u64));
            obj
        }
        Err(e) => {
